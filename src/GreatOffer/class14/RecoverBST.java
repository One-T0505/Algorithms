...
public class RecoverBST {
    ...
    // 思路：首先我们要确定出两个错序的结点。我们将这棵树的中序写出来，第一次出现降序的第一个结点，和最后一个降序
    // 的第二个结点就是两个错误的结点。比如：
    //
    //                   10
    //                 /     \
    //                6       18              将9和21交换位置，然后中序序列是：
    //              /  \      / \             3  6  7  8  21  10  12  14  18  9  22  24
    //             3    9    14  21
    //                 /     /    \           按照上面说的规则，两个交换的结点可以正确地推出：9、21
    //                8     12    24
    //               /            /
    //              7            22
    // 有一种情况就是，将中序遍历序列相邻的元素交换，这样只会产生一个降序，所以才有了上面的总结，上面的总结就是为了
    // 应对所有的情况而定制的判断条件


    // 整个算法可以达到 时间复杂度O(N)  空间复杂度O(1)
    ...
    // 上面的代码只是将错误结点的值交换即可，而实际工程上是不会这样做的，工程上一定是将两个错误结点的环境
    // 全部修改好，修改好相关指针的指向。但是这样的算法是非常难的，因为涉及到的清况太多，一共有14种，下面
    // 是左老师手写的代码，有兴趣慢慢研究。
