...
public class BucketSort {
    ...
    private static void radixSort(int[] arr, int l, int r, int maxDigits) {
        ...
        for (int d = 1; d <= maxDigits; d++) {  // d等于1时代表按照个位来排序
            ...
            // 举例,假如此时是按照个位来操作：arr = {101, 267, 133, 104, 352, 601} count本来是这样的 = [0, 2, 1, 1, 1, 0, 0, 0, 0, 0] 表示个位数分别为0-9的元素
            // 有几个。经过上面的累加处理，count就成这样的了：count = [0, 2, 3, 4, 5, 5, 5, 5, 5, 5]。这样理解：个位数<=0的有0个，个位数<=1的有2个，个位数<=2的有3个
            // 个位数<=3的有4个。。。 现在对arr从右至左遍历：第一个是601，个位为1，那就说明按照传统的基数排序进队列来说，601一定是1号队列的最后一个，那么出队列排好序后，
