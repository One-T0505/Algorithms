...
public class  SubSequence {
    ...
    public static List<String> subSequence(String s){
        ...
        process1(chars, 0, res, pre);
        return res;
    }

    // chars是固定参数，一直不变； index表示当前要决定chars[index]的字符是否要，而0..index-1已经做好决定了，
    // 并且其结果保存在了pre中
    private static void process1(char[] chars, int index, List<String> res, String pre) {
        if (index == chars.length){
            res.add(pre);
        }else {
            // 这个递归子过程index变成index + 1了，说明index处已经做好决定了，我们发现pre这个参数没有变化，
            // 说明这个递归子过程数不要chars[index]处字符的递归
            process1(chars, index + 1, res, pre); // 不选择chars[index]位置的字符
            String attend = pre + chars[index];
            process1(chars, index + 1, res, attend);
        }
    }
    // ============================================================================================
    ...
    public static HashSet<String> subSequenceNorepeat(String s){
        ...
        process2(chars, 0, res, pre);
        return res;
    }

    private static void process2(char[] chars, int index, HashSet<String> res, String pre) {
        if (index == chars.length){
            res.add(pre);
        }else {
            // 这个递归子过程index变成index + 1了，说明index处已经做好决定了，我们发现pre这个参数没有变化，
            // 说明这个递归子过程数不要chars[index]处字符的递归
            process2(chars, index + 1, res, pre); // 不选择chars[index]位置的字符
            String attend = pre + chars[index];
            process2(chars, index + 1, res, attend);
        }
    }
    // ============================================================================================
    ...
    public static List<String> permutationV1(String s){
        ...
        for (char c : chars)
            rest.add(c);
        String pre = "";
        process3(rest, pre, res);
        return res;
    }

    // chars[0..index-1]已经做好决定了，不用管了。chars[index..]都有机会到index位置上
    // 如果index到了终止位置，chars当前的样子就是一种结果
    private static void process3(List<Character> rest, String pre, List<String> res) {
        if (rest.isEmpty())
            res.add(pre);
        else {
    ...
    public static List<String> permutationV2(String src){
        ...
        process4(chars, 0, res);
        return res;
    }

    // 从pos位置开始做决策，0..pos-1位置已经决策好了，并且结果就在chars中
    private static void process4(char[] chars, int pos, List<String> res) {
        if (pos == chars.length)
            res.add(String.valueOf(chars));
        else {
            for (int i = pos; i < chars.length; i++) {
                swap(chars, pos, i);
                process4(chars, pos + 1, res);
                swap(chars, pos, i);
            }
        }
    }
    ...
    private static void process5(char[] chars, int index, ArrayList<String> res){
        if (index == chars.length)
            res.add(String.valueOf(chars));
