...
public class PokerGame {
    ...
    public static int winnerV2(int[] arr){
        if (arr == null || arr.length == 0)
            return -1;
        int N = arr.length;
        int[][] firstDp = new int[N][N];  // 用于记录先手递归的缓存表
        int[][] secondDp = new int[N][N];  // 用于记录后手递归的缓存表
        // 将缓存表所有值填为-1，这样就可以根据是否为-1判断该结果是否计算过
        for (int row = 0; row < N; row++) {
            Arrays.fill(firstDp[row], -1);
            Arrays.fill(secondDp[row], -1);
        }
        int score1 = first2(arr, 0, N - 1, firstDp, secondDp);
        int score2 = second2(arr, 0, N - 1, firstDp,secondDp);
