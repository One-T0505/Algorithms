...
public class Dijkstra {
    public static HashMap<GraphNode, Integer> dijkstraV1(GraphNode src){
        ...
        HashSet<GraphNode> lock = new HashSet<>();
        GraphNode minDis = selectNode(distance, lock);
        while (minDis != null){
            int dis = distance.get(minDis);
            for (GraphEdge edge : minDis.edges){
    ...
    private static GraphNode selectNode(HashMap<GraphNode, Integer> distance, HashSet<GraphNode> lock) {
        GraphNode res = null;
        int minDistance = Integer.MAX_VALUE;
        for (Map.Entry<GraphNode, Integer> entry : distance.entrySet()){
    ...
    public static HashMap<GraphNode, Integer> dijkstraV2(GraphNode src, int size){
    ...
    public static class DynamicHeap {
        private GraphNode[] heap;  // 这个就是实现堆序的数组

        // 记录当前结点在heap的索引位置，如果为-1，说明该结点，曾经入过堆，并且已经弹出了，也就是说该结点锁住了，
        // 源结点到该结点的距离不再更新了。在图中，可能会出现回溯的边指向已经不需要再更新的结点了。
        private HashMap<GraphNode, Integer> index;
        private HashMap<GraphNode, Integer> distance;
        ...
        public void addOrUpdateOrIgnore(GraphNode cur, int dis) {
            // 1.如果根本没进过堆
            if (!isEntered(cur)){
                heap[size] = cur;
                index.put(cur, size);
        ...
        // 表示进来过，但此刻是否还在堆上
        private boolean isInHeap(GraphNode node){
            return isEntered(node) && index.get(node) != -1;
        }

        private void upHeapify(GraphNode src, int pos) {
