...
public class RingList {
    ...
    // 第一步找到入环结点后，应该处理下一个问题了。下面的方法用于已知两个单链表都无环的情况下，返回第一个相交结点，若不相交则返回null.
    // 如果两个单链表相交，那么最后一个结点内存地址必然相同，若不相同则不相交。若相交，那么最后一个结点也只是相交的最后一个结点，
    // 我们需要找到相交的第一个结点。这样找：假设链表1长度为50，链表2长度为30，那就让h1先走20步，然后两个一起走，第一个相同的地方
    // 就是第一个相交的地方
    ...
    // 如果一个链表有环，一个链表无环，这样是不可能相交的。所以只需要在讨论下，两个链表都有环的情况下，如何找出相交的第一个结点
    // 两个链表都有环的情况也要再细分成三种情况：
    // 1.两个链表各自玩各自的，没有任何关系
    //              ___                 ___
    //             /   \               /   \
    //        _____\___/          _____\___/
